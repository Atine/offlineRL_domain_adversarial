...
class Workspace(object):
    ...
    def train_offline(self):
        ...
        load_offline_dataset_into_buffer(
            [Path(self.cfg.offline_dir), Path(self.cfg.offline_dir_dis)],
            self.replay_buffer,
            self.cfg.frame_stack,
            self.cfg.replay_buffer_size,
        )
        ...
        while train_until_step(self.global_step):
            ...
            metrics_curr = self.agent.update(
                self.replay_buffer, self.global_step, self.cfg.batch_size
            )
...
def main(cfg):
    ...
    # use "distracting_mode" to control which data to learn directly
    # expected to see distracting mode is True
    if not cfg.distracting_mode:
        raise Exception(
            f"expected to get distracting_mode=True, "
            f"got {cfg.distracting_mode}"
        )
    if cfg.offline_dir == "...":    # only overwrite if not specified
        cfg.offline_dir = os.path.join(
            f"{os.path.expanduser('~')}/.v_d4rl/vd4rl/main/",
