...
DISTRACTING_EVAL_FORMAT = [
    ("step", "S", "int"),
    ("episode_length", "L", "int"),
    ("episode_reward_mean", "R", "float"),
    ("easy_episode_reward_mean", "EER", "float"),
    ("medium_episode_reward_mean", "MER", "float"),
    ("hard_episode_reward_mean", "HER", "float"),
    ("fixed_easy_episode_reward_mean", "FEER", "float"),
    ("fixed_medium_episode_reward_mean", "FMER", "float"),
    ("fixed_hard_episode_reward_mean", "FHER", "float"),
    ("total_time", "T", "time"),
    ("ETA", "ETA", "time"),
]
...
class MetersGroup(object):
    ...
    def _remove_old_entries(self, data):
        ...
        with self._csv_file_name.open("w") as f:
            writer = csv.DictWriter(
                f, fieldnames=sorted(data.keys()), restval=0.0
            )
            writer.writeheader()
            for row in rows:
                writer.writerow(row)
    ...
    def _format(self, key, value, ty):
        if ty == "int":
            value = int(value)
            if key == "S":
                return f"{key}: {value: >7}"
            else:
                return f"{key}: {value}"

        elif ty == "float":
            return f"{key}: {value: >6.02f}"
...
class Logger(object):
    def __init__(self, log_dir):
        ...
        train_formatting = OFFLINE_TRAIN_FORMAT
        eval_formatting = DISTRACTING_EVAL_FORMAT
        self._train_mg = MetersGroup(
            log_dir / "train.csv", formating=train_formatting
        )
        self._eval_mg = MetersGroup(
            log_dir / "eval.csv", formating=eval_formatting
        )
        self._sw = None
