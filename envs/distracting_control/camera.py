...
def get_lookat_xmat_no_roll(agent_pos, camera_pos):
    ...
    # NOTE(austinstone): This method leads to wild oscillations around
    # the north and south polls.
    # For example, if agent is at (0., 0., 0.) and the camera is at
    # (.01, 0., 1.), this will produce a yaw of 90 degrees whereas if the
    # camera is slightly adjacent at (-.01, 0., 1.) this will produce a yaw
    # of -90 degrees. I'm not sure what the fix is, as this seems like the
    # behavior we want in all environments except for reacher.
...
class DistractingCameraEnv(control.Environment):
    ...
    **NOTE**: This wrapper should be applied BEFORE the pixel wrapper to
    make sure the camera pose changes are applied before rendering occurs.
    """
    ...
    def setup_camera(self):
        ...
        self._start_h_angle = start_h_angle
        self._start_v_angle = start_v_angle
        # Scale the velocity by the starting radius. Most environments have
        # radius 4, but this downscales the velocity for the envs with
        # radius < 4.
        ...
        self._min_radius = max(
            start_r - start_r * self._max_zoom_in_percent, 0.0
        )
        self._max_radius = start_r + start_r * self._max_zoom_out_percent

        # Decide the starting position for the camera.
        self._h_angle = self._random_state.uniform(
            self._min_h_angle, self._max_h_angle
        )

        self._v_angle = self._random_state.uniform(
            self._min_v_angle, self._max_v_angle
        )

        self._radius = self._random_state.uniform(
    ...
    def reset(self):
        ...
        # if self.fixed_angle is None or self.fixed_angle is False
        if self.fixed_angle in [None, False]:
            self._apply()
        else:
            self._apply_fixed_angle(self.fixed_angle)
        return time_step
    ...
    def _apply(self):
        ...
        cart_cam_pos = sphere2cart(
            [self._radius, self._h_angle, self._v_angle]
        )
        ...
        if self._camera_type == "fixed":
            lookat_mat = lookat_method(
                self._cam_initial_lookat_point, cart_cam_pos
            )
        else:
            # Go from agent centric to world coords
            cart_cam_pos += self._env.physics.named.data.subtree_com[1]
            lookat_mat = lookat_method(
                get_lookat_point(self._env.physics, self._camera_id),
                cart_cam_pos,
            )
    ...
    def _apply_fixed_angle(self, degrees):
        if not self._camera_type:
            self.setup_camera()

        final_h_angle = self._start_h_angle + (degrees * np.pi / 180)
        # final_v_angle = self._start_v_angle + (0 * np.pi / 180)
        cart_cam_pos = sphere2cart(
            [self._start_r, final_h_angle, self._start_v_angle]
            # [self._start_r, final_h_angle, final_v_angle]
        )
        # cart_cam_pos += self._env.physics.named.data.subtree_com[1]
        # lookat_mat = get_lookat_xmat_no_roll(
        #     get_lookat_point(self._env.physics, self._camera_id),
        #     cart_cam_pos)
