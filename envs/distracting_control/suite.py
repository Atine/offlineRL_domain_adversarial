...
def load(
    ...
    # seeding the distractions
    fixed_distraction=False,
    background_seed=None,
    color_seed=None,
    camera_seed=None,
    # individual distraction only,
    apply_background_dis=True,
    apply_camera_dis=True,
    apply_color_dis=True,
):
    """Returns an environment from a domain name, task name and optional
       settings.
    ...
    Adding a difficulty will configure distractions matching the reference
    paper for easy, medium, hard.
    ...
    Args:
      ...
      background_dataset_path: String to the davis directory that contains the
        video directories.
      background_dataset_videos: String ('train'/'val') or list of strings of
                                 the DAVIS videos to be used for backgrounds.
      ...
      pixels_only: Boolean controlling the exclusion of states in the
                   observation.
      pixels_observation_key: Key in the observation used for the rendered
                              image.
      env_state_wrappers: Env state wrappers to be called before the
                          PixelWrapper.
...
def distracting_wrapper(
    ...
    if (difficulty or background_kwargs) and apply_background_dis:
        background_dataset_path = (
            background_dataset_path or suite_utils.DEFAULT_BACKGROUND_PATH
        )
        final_background_kwargs = dict(
            fixed=fixed_distraction, seed=background_seed
        )
        if difficulty:
        ...
        # finally create env
        env = background.DistractingBackgroundEnv(
            env, **final_background_kwargs
        )

    # Apply camera distractions.
    if (difficulty or camera_kwargs) and apply_camera_dis:
        final_camera_kwargs = dict(
            camera_id=render_kwargs["camera_id"],
            fixed=fixed_distraction,
            seed=camera_seed,
        )
    ...
    if (difficulty or color_kwargs) and apply_color_dis:
    ...
    # changes from the distraction wrapper are applied to the MuJoCo
    # environment before the rendering occurs.
    env = pixels.Wrapper(
        env,
